...
void GameScene::loop()
{
  ...
  while (42)
  {
    ...
    {
      b2World_Step(*this->worldId, this->TIME_STEP, VELOCITY_ITERATIONS);
      refresh = true;
      this->update();
    }
...
{
  this->worldDefinition = new b2WorldDef(b2DefaultWorldDef());
  this->worldDefinition->gravity = this->gravity;

  this->worldId = new b2WorldId();
  *(this->worldId) = b2CreateWorld(this->worldDefinition);
  ...
  for (GameObject *object : this->objects)
  {
    if (object->isActive())
    {
      ...
      object->createShape(objectRigidBodyId, object->getHalfWidth(), object->getHalfHeight());
      b2BodyId *bodyId = new b2BodyId(objectRigidBodyId);
      object->setRigidBody(bodyId);
